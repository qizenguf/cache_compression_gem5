...
if [ ! -d "$OUTPUT_DIR/$BENCHMARK/car2b" ]; then
mkdir $OUTPUT_DIR/$BENCHMARK/car2b
fi
$GEM5_DIR/build/ALPHA/gem5.opt --outdir=$OUTPUT_DIR/$BENCHMARK/car2b $GEM5_DIR/configs/example/spec06_se_4mix.py --benchmark=$BENCHMARK --cpu-type=atomic -n 4 --cpu-clock=3GHz --mem-size=2GB --mem-channels=2 --mem-type=DDR3_1600_8x8 --caches --l1i_size=32kB --l1i_assoc=8 --l1d_size=32kB --l1d_assoc=8 --l2cache --l2_size=256kB --l2_assoc=8 --l3cache --l3_size=16MB --l3_assoc=32 --l3_tags=2 --checkpoint-dir=/home/qizeng/gem5/cpts/  --checkpoint-restore=$WARM_LEN --restore-with-cpu=detailed --maxinsts=$RUN_LEN --warmup-insts=1000000000
